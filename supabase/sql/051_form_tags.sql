-- Clasificación administrativa de formularios. Las etiquetas permiten agrupar
-- instrumentos compatibles sin alterar preguntas ni respuestas históricas.
alter table public.forms
  add column if not exists tags text[] not null default '{}'::text[];

create index if not exists idx_forms_tags on public.forms using gin (tags);

-- Conservar la clasificación cuando un formulario se duplica para una nueva sesión.
...
begin
  insert into public.forms (
    ...
    allows_partial_save, deadline_at, created_by, purpose, tags
  )
  select title, description, instructions_popup, 'draft', false,
    allows_partial_save, deadline_at, p_created_by, purpose, tags
